...
struct PageChunk : public CommonChunk
{
	...
	__forceinline__ __device__ __host__ void* getPage(memory_t* memory, uint64_t start_index, index_t chunk_index, uint32_t page_index)
	{
		return reinterpret_cast<memory_t>(Base::getData(memory, start_index, chunk_index)) + (page_index * page_size);
	}
	...
	{
		auto chunk = reinterpret_cast<PageChunk*>(Base::getMemoryAccess(memory, start_index, chunk_index));
		return QI::getQueueIndex(chunk->page_size);
	}
